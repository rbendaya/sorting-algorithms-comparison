...
void sorter::heap_sort_implement(vector<int64_t> &v)
{
    ...
    for (uint64_t i = n - 1 + 1; i > 0; --i) // + 1 is added in the loop then deducted so uint can be used
    {
        ...
        // then again heap-ify but excluding the last element after each iteration
        heap_ify(v, i - 1, 0);
    }
}

void sorter::heap_ify(vector<int64_t> &v, uint64_t n, uint64_t i)
{
    ...
    if (largest != i)
    {
        ...
        // recursive heap-ify to the subtree that might be effected by change in the root node
        heap_ify(v, n, largest);
    }
}
