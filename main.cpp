...
int main(int argc, char *argv[])
{
    ...
    // the below code is here to avoid an error when we first run the code as argv is NULL before user inputs commands
    if (argv[1] == NULL)
    {
    ...
    // Incase users inputs more than one command line parameter
    if (argc > 2)
    ...
    int64_t list_element;        // Variable to store the numbers from the file
    vector<int64_t> user_vector; // Vector to store those elements/ user's list
